...
public interface FeedLikeJpaRepository extends JpaRepository<FeedLikeEntity, Long> {
    ...
    @Query("select fl from FeedLikeEntity fl where fl.feedEntity.feedId=:feedId and fl.userEntity.id=:userId")
    Optional<FeedLikeEntity> findOneByFeedIdAndUserId(@Param("feedId") Long feedId, @Param("userId") Long userId);
